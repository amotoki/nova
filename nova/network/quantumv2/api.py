...
from nova.network import flags
...
class API(base.Base):
    ...
    def _get_port_id_by_fixed_address(self, client,
                                      instance, address):
        # Search a target port from fixed_address and instance
        zone = 'compute:%s' % FLAGS.node_availability_zone
        search_opts = {'device_id': instance['uuid'],
                       'device_owner': zone}
    ...
    def _setup_ports_dict(self, client, project_id=None):
        search_opts = {'tenant_id': project_id} if project_id else None
    ...
    def _format_floating_ip_model(self, fip, pool_dict, port_dict):
        ...
        # 'intance_uuid' is stored in device_id attribute of port object.
        if fip['port_id']:
    ...
    def _get_floating_ip_pool_id_by_name_or_id(self, client, name_or_id):
        ...
        if re.match(UUID_PATTERN, name_or_id):
    ...
    def allocate_floating_ip(self, context, pool=None):
        ...
        client = quantumv2.get_client(context)
        # FLAGS.default_floating_pool must be either name or id
        # of a network for a floating IP pool.
    ...
    def _get_floating_ip_by_port_id(self, client, port_id):
        """Get floatingip from associated quantum port ID"""
        data = client.list_floatingips(port_id=port_id)
        fips = data['floatingips']

        # At the moment list_floatingips() does not filter by port_id.
        # So filter by port_id by hand.
        # TODO(amotoki): It should be fixed by quantum (or quantumclient).
        fips = [x for x in fips if x['port_id'] == port_id]

        if len(fips) == 0:
            raise exception.FloatingIpNotFoundForAddress(address=address)
        elif len(fips) > 1:
            # Quantum checks the specified port_id is already associated.
            # This exception will not be raised unless quantum has a bug.
            raise exception.FloatingIpMultipleFoundForPort(id=port_id)
        fip = fips[0]
        return fip
